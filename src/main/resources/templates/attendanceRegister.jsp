...
<nav class="navbar">
    ...
    <table>
        ...
        <tbody>
            <tr th:each="are, status : ${ar}">
                <td th:text="${status.index + 1}"></td>
                <td th:text="${are.session}"></td>
                <td th:text="${arext[status.index][0]}"></td>
                <td th:text="${arext[status.index][1]}"></td>
                <td th:text="${arext[status.index][2]}"></td>
                <td>
                    <form th:action="@{'/attendance/presents/' + ${are.id} }">
                        <input type="submit" class="btn-show" value="VIEW">
                    </form>
                </td>
                <td th:text="${arext[status.index][3]}"></td>
                <td>
                    <form th:action="@{'/attendance/absents/' + ${are.id} }">
